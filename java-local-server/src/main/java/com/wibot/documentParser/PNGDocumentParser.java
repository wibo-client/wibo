 ...
 * PNG Document Parser
 * 
 * 1）读取PNG图像 2）把图像传给远端的一个接口，用来做图片的文字识别， 3）把识别好的文字，叠加那个图片，拼装成一个markdown的一段
 */

@Service
public class PNGDocumentParser extends AbstractDocumentParser {
    ...
    private final Logger logger = LoggerFactory.getLogger(PNGDocumentParser.class);

    private int getMaxWidth() {
        return systemConfigService.getConfig(SystemConfigService.CONFIG_PNG_MAX_WIDTH, Integer.class, 800);
    }

    private int getMaxHeight() {
        return systemConfigService.getConfig(SystemConfigService.CONFIG_PNG_MAX_HEIGHT, Integer.class, 800);
    }

    private boolean getPngUseOCR() {
