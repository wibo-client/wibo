...
// public Flux<ServerSentEvent<String>> stream(@RequestBody RequestData
// requestData, HttpServletResponse response) {
...
// logger.info("Received request with input: {}, type: {}, path: {}", input,
// type, path);
...
// ServerSentEvent.builder(data).build()).onErrorResume(WebClientResponseException.class,
// ex -> {
// if (ex.getStatusCode() == HttpStatus.UNAUTHORIZED) {
// return Flux.just(ServerSentEvent.builder("Unauthorized: 请在右上角的 【管理界面】
// 中输入阿里云的大模型AK").build());
// }
...
// DocumentIndexInterface documentIndexInterface =
// pathBasedIndexHandlerSelector.selectIndexHandler(path);
// SearchQuery searchQuery = new SearchQuery();
// searchQuery.setOriginalQuery(input);
// searchQuery.setPathPrefix(path);
// searchQuery.setTopN(20);

// List<SearchDocumentResult> documentPartList =
// documentIndexInterface.searchWithStrategy(searchQuery);
// List<SearchResult> searchResults = processDocumentPartList(documentPartList,
// 10);
...
// List<SearchDocumentResult> documentPartList =
// documentIndexInterface.searchWithStrategy(searchQuery);
// List<SearchDocumentResult> rerankedDocumentPartList =
// documentRerankInterface.rerank(documentPartList, input);
// List<SearchResult> searchResults =
// processDocumentPartList(rerankedDocumentPartList, 10);

// logger.info("Search with rerank completed. Found {} results.",
// searchResults.size());
...
// private List<SearchResult> processDocumentPartList(List<SearchDocumentResult>
// documentPartList, int limit) {
...
// sb.append("## file
// index:").append(fileNumber.getAndIncrement()).append("\n");
// sb.append("### File Path \n");
...
// singletonLLMChat.getChatClient().prompt(prompt).stream();
...
// documentIndexInterface.searchWithStrategy(searchQuery);
// List<SearchDocumentResult> rerankedDocumentPartList =
// documentRerankInterface.rerank(documentPartList,
// input);
...
// Optional<DocumentDataPO> docData =
// documentDataRepository.findById(documentDataId);
...
// if (currentLength + combinedContent.length() + partHeader.length() >
// MAX_BATCH_SIZE_5000) {
// contextBuilder.append(
// combinedContent.substring(0, MAX_BATCH_SIZE_5000 - currentLength -
// partHeader.length()));
...
// logger.debug("Added content from document {}, current length: {}",
// documentId, currentLength);
// }
...
// .doOnError(error -> logger.error("Error in chat stream: ",
// error)).doOnComplete(() -> {
...
// int index = 1;
// // referDocs 去重
// List<DocumentDataPO> referDocsDistinct =
// referDocs.stream().distinct().toList();
// for (DocumentDataPO docData : referDocsDistinct) {
// combinedOutput.append("doc ").append(index).append(":
// ").append(docData.getFilePath())
// .append("\n");
// combinedOutput.append("\n");
// if ((index++) > 3) {
// break;
